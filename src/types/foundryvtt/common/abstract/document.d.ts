...
export default class Document {
    ...
    /**
     * This function applies migration to the source data based on the core version this data originated from.
     * It must be given nonpartial data matching the schema it had in the core version it is coming from.
     * The core version is recorded in the `_stats` field. If the document model doesn't have a `_stats` field,
     * the given source data is not modified. If the document model has a `_stats` field and the given source data
     * doesn't contain a `_stats` field, the data is assumed to be pre-V10, when the `_stats` field didn't exist yet.
     * Therefore the `_stats` field must not be stripped from the data before it is saved!
     * This function does not change the `_stats`: the core version of the data is _not_ set to the current version!
     * If the source doesn't contain `_stats` (and therefore is pre-V10), it is added with all its fields set to `null`
     * to prevent the document that will be created from the source being given fresh, current-version `_stats`.
     * @param {object} source    The nonpartial document data which comes from a trusted source.
     * @returns {object}         The `source` argument.
     * @internal
     */
    static _migrateSource(source: object): object;
    /**
